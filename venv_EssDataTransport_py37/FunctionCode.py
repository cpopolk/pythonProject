...
#         print(stored_path)
          ...
          with open(Log_stored_path, 'a') as file:
           file.write("\n failed to export data/disconnected communication")
           #file.write(stored)
          self.Transfer_Time.insert(0, str(datetime.datetime.now()))
          self.Response_Result.insert(0, "failed to export data/disconnected communication")
#         print('failed to export data/disconnected communication')
...
#          file.write(stored)
          self.Transfer_Time.insert(0, str(datetime.datetime.now()))
          self.Response_Result.insert(0, "failed to connect postgresql")
...
# in  사용법 , series를 list화 : 'AI: BAT 2 Usable State of Charge' in DGPC_Database_df['DB_TAG'].to_list()
           ...
           if 'AI: BAT 2 Usable State of Charge' in DGPC_Database_df['DB_TAG'].to_list():
               ...
               BMS_EMS_DigitCorrection = [BMS_MergredDataframe_1, BMS_MergredDataframe_2, EMS_MergredDataframe_1,
                                          EMS_MergredDataframe_2]
               ...
               Export_Dataframe = [BMS_MergredDataframe_1, BMS_MergredDataframe_2, EMS_MergredDataframe_1,
                                   EMS_MergredDataframe_2, ETC_List_1_df, ETC_List_2_df]
           # DER 1개일 경우
           else:
               Common_list_df = pd.DataFrame(self.Common_list)
               BMS_List_1_df = pd.DataFrame(self.BMS_List_1)
...
#               print(EMS_MergredDataframe_1)
    ...
    # Value 자릿수 변환
               ...
               for datablock in BMS_EMS_DigitCorrection:
                   ...
                   for row in datablock['TYPE']:
                       ...
                       if row == 'DECIMAL(4,1)':
                           print(datablock)
                           beforeTypechanged = round(float(datablock.loc[i,'Value']),1)
                           print(beforeTypechanged)
                           datablock.loc[i, 'Value'] = beforeTypechanged  # loc 데이터프레임 컬럼명으로 구분 , iloc 컬럼숫자로 구분
                           i = i + 1
    ...
    #              print(ETC_List_1_df)
          ...
          Log_stored_path = "{}/Data Log/{}.txt".format(filePath, filename)
          with open(Log_stored_path, 'a') as file:
              file.write("failed to take Data (데이터 취득/가공실패)")
          self.Transfer_Time.insert(0, str(datetime.datetime.now()))
          self.Response_Result.insert(0, "failed to take Data (데이터 취득/가공실패)")
...
#        print(outputData)
        ...
        #      print(stored_path)
        with open(Log_stored_path, 'a') as file:
            file.write("No DataList be created(데이터리스트를 생성하지 못함)")
        self.Transfer_Time.insert(0, str(datetime.datetime.now()))
        self.Response_Result.insert(0, "No DataList be created(데이터리스트를 생성하지 못함)")
...
#      print(self.url)
        ...
        with open(Log_stored_path, 'a') as file:
            file.write("failed to AES(암호화 실패")

    #      print('failed to AES(암호화 실패)' )
        self.Transfer_Time.insert(0, str(datetime.datetime.now()))
        self.Response_Result.insert(0, "failed to AES(암호화 실패")
...
# HMI에 표시
      ...
      print('failed to Transport data(외부전송실패)' )
      self.Transfer_Time.insert(0, str(datetime.datetime.now()))
      self.Response_Result.insert(0, "failed to Transport data(외부전송실패)")
